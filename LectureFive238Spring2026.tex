...
f_{p|\text{not fair}}(p) = 1 \qt{for every $p \in [0, 1]$}.   
\end{equation}
...
\begin{equation*}
  \P \left\{\text{Model}~ i~ |~ \text{data}  \right\}  \qt{for $i = 1,
    2$}. 
\end{equation*}
...
\begin{equation}\label{prior_mackay}
  \P \{\text{Model}~i\} = \frac{1}{2} \qt{for each $i = 1, 2$}
\end{equation}
...
\P \left\{\text{data} \right\} = \sum_{i=1}^3\P
  ...
  \begin{equation*}
    \P\{\text{Model}~i\} = \frac{1}{3} \qt{for each $i = 1, 2, 3$}, 
  \end{equation*}
  we obtain
  \begin{equation*}
